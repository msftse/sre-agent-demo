...
`scripts/deploy-teams-bridge.sh` invokes this connector automation after Function health succeeds. Connector creation therefore requires no SRE portal work and puts no secret in Git or Terraform state. Manual custom-app sideloading into Teams remains the only portal checkpoint because tenant-wide app catalog permissions were intentionally not requested.
...
Stage 12 is complete. Teams transport, threading, secure MCP exposure, and automated SRE connector provisioning are ready. Stage 13 can validate the GitHub connector capability boundary without activating the incident.