...
The incident is not active. Start it in GitHub under **Actions > Start Demo > Run workflow**, select **Confirm creation**, and run it. The starter validates and merges its setup PR, then deploys the incident automatically. After Azure SRE Agent creates its remediation PR, review and merge that PR. `Deliver Demo to AKS` automatically restores the application and the agent publishes the final RCA. See [docs/stages/17-approval-rejection-rehearsal.md](docs/stages/17-approval-rejection-rehearsal.md).
