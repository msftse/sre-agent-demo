...
Organization policy blocks PR creation with the default workflow token. Repository secret `STAGE17_GITHUB_TOKEN` therefore supplies an operator-scoped credential only to the PR-creation step. The default workflow token creates the scenario branch. The starter has no review or merge API call, and `incident-demo` branch protection still requires the user-performed merge. Prefer a fine-grained token or GitHub App limited to this repository with pull-request write access.
