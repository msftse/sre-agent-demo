...
The incident is not active. Start it in GitHub under **Actions > start-demo > Run workflow**, select **Confirm creation**, and run it. The starter validates and merges its setup PR automatically; approve the resulting protected `demo` deployment to activate the incident. After Azure SRE Agent creates its remediation PR, merge that PR and approve the protected recovery deployment. `deliver-demo` restores the application and the agent publishes the final RCA. See [docs/stages/17-approval-rejection-rehearsal.md](docs/stages/17-approval-rejection-rehearsal.md).
