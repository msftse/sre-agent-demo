...
10. **Create the remediation change.** Create a unique branch named `sre/field20-checkout-<incident-id>`. Push the source and regression-test changes in one coherent commit, then open a pull request to `main`. Include evidence, root cause, changed behavior, validation expectations, risk, rollback notes, and exactly two hidden markers: `<!-- sre-thread-id: <current-sre-thread-resource-id> -->` for SRE continuation and `<!-- teams-thread-id: <current-incident-id> -->` for the existing Teams timeline. The SRE marker must be the canonical chat thread resource ID for the current conversation, not the Azure alert ID or `incidentStatus.incidentId`; confirm that reading `/api/v1/threads/<sre-marker>/messages` returns this conversation's existing messages. The Teams marker must equal the branch suffix and must resolve through `get_incident_thread`; an empty auxiliary SRE incident thread is not valid SRE correlation.
