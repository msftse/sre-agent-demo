...
1. Before any source write, call `northstar-teams_post_incident_update` once with `incident_id` set to the current Azure Monitor incident ID to create the incident timeline. Include the incident title and ID, severity, fired time, affected resource scope, SRE investigation link or thread ID, and status `Investigation started`.
...
3. Use `northstar-teams_reply_incident_thread` with the same Azure Monitor `incident_id` for material state changes only:
